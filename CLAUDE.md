...
│   ├── kits/               # kit indexes and make_*.py kit builders
│   ├── export_assets.py    # approved art/final -> assets/ (atlases + JSON indexes)
│   └── requirements.txt    # Pillow
├── assets/                 # what Godot imports; generated by tools/export_assets.py, never edited by hand
│   ├── characters/<name>/<name>_{s,e,n,w}.png
│   ├── enemies/<name>/<name>_{s,e,n,w}.png
│   └── tilesets/           # one atlas PNG + JSON index per kit; terrain/ holds the Wang sheets
├── scenes/                 # Phase 1+: .tscn by feature (player/, world/, enemies/, npcs/, ui/), each scene's .gd beside it
└── resources/              # Phase 1+: .tres data (TileSets, items, enemies, dialogue, area settings)
```
...
- **`tools/build_tileset.py`**: generates `resources/tilesets/meadow.tres` from the exported atlases: stable source ids, a corner-matching terrain set (grass, path, water, bank, leaves) with weighted fill variants, collision (physics layer 0 `world`, 1 `water`) and y-sort origins at each stamp's base. Collision rule: houses, the gable, the hollow oak and dungeon walls block whole tiles; trees, bushes, stones, the shrine, fences and ruins block only at their **footprint** (`tools/footprint.py`, the solid base), so Awa walks behind them; a baked wood blocks at its trunks and its solid canopy interior (`tools/kits/forest16_meta.json`, written by `paint_forest.py`). Re-run after `export_assets.py`; hand edits to the TileSet are overwritten.
...
- Textures import lossless with no mipmaps (Godot's 2D defaults); keep it that way.

**Input map (gamepad-first; Xbox layout, keyboard fallback):**

| Action | Gamepad | Keyboard |
|---|---|---|
| `move_left/right/up/down` | left stick (deadzone 0.25), d-pad | WASD, arrows |
| `interact` | A | E, Enter |
| `attack` (walking stick) | X | J, Space |
| `use_item` | Y | K |
| `cancel` | B | Q, Backspace |
| `diary` | Back / View | Tab |
