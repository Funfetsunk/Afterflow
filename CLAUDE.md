...
- **`tools/make_stamp.py`** / **`tools/paint_tiles.py`**: turn a prop into a fixed 16px tile stamp (registry `tools/kits/stamps.json`) and paint stamps, fence runs (`tools/kits/fence16.json`) or crumbled ruin runs (`tools/kits/ruin16.json`, built by `tools/kits/make_ruin16.py`) into a mock layout as grid cells. `tools/kits/make_shrine16.py` draws the shrine sprite in code.
