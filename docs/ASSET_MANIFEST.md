...
|---|---|---|---|
| 2026-09-21 → 2026-10-21 | Tier 1 (Pixel Apprentice) | 194 | 859 of 2,000 were already used this cycle before Afterflow started; 1,140 left on 2026-09-25; 1,000 left after north-star round 1; 990 after 16px grass-path; 981 after the 16px river; 977 after Awa v2; 971 after trees; 962 after the forest tileset attempt; 958 after shrine round 1; 954 after shrine round 2; 950 after the miller; 946 after the meadow creature |
...
| `npc_villager01` | The miller, 16-bit | size 28 (figure 26–28px) | opt1 `53df9e8b-b6b0-4616-8bd4-760574e6726c`<br>opt2 `c372f879-8411-4bb4-9848-841fb0888719`<br>opt3 `7a6ad63d-c560-4c30-8139-18d0fe596384`<br>opt4 `baf7f17d-99e8-4fc8-be51-31833cb52e47` | 4 | **opt1** | 4 | approved | **Approved by Tom, 2026-09-25** (opt1, as generated). Final files `art/final/npc_villager01_{s,e,n,w}.png`. Prompt: "stout middle-aged village miller, kind but guarded expression, short grey-brown beard, flat cloth cap, rolled-up shirt sleeves, flour-dusted pale apron over a moss green waistcoat, brown trousers, sturdy boots" + style suffix. `create_character` standard, `size: 28`, `high top-down`, `single color black outline`, `basic shading`, `low detail`, 4 directions; opt1–2 `default` preset, opt3–4 `cartoon`. Raw `art/raw/npc/npc_villager01_opt<n>_<d>.png`, remapped `art/final/npc/`. None came out stout. opt1: flat cap, grey beard, long pale apron, consistent across directions (best read as a miller). opt2: white cap and apron (reads as a baker or chef). opt3: the beard reads as a face mask. opt4: dark hair and moustache, youngest-looking. Reviews: `art/mocks/review_v2_miller_sheet.png` (all directions beside Awa), `review_v2_miller_6x.png` (in the village) |
...
| `obj_shrine01` | Shrine, 16-bit (ancient-future) | 2×3 tile stamp (28×42 / 28×39 px) | round 1 pixflux 00f62151, 479ba84a, 9bf56b56, 49f7e239; round 2 17d39b41, 814a7fbd, 2acffbb5, 79bbb32b | 8 generated + 2 hand-drawn | `shrine_monolith` | 8 | approved | **Approved by Tom, 2026-09-25 (monolith; the ring was not chosen, and its stamp was removed, but `make_shrine16.py` still draws it).** Round 1 (weathered stone niche, seeds 1–4): Tom rejected both cleaned options ("make it look a little more futuristic"). Round 2 (seeds 11–14, "smooth pale monolith … ancient-future stone … glowing seams" and "smooth pale stone ring on a hexagonal plinth … glowing light floating in the centre" + style suffix, 32×48, `no_background`, north star as `color_image_url`): washed out, mostly no outlines, so not used. The final designs are **drawn in code** by `tools/kits/make_shrine16.py` (in the palette, `2e222f` outlines, lit from the top left): a two-tier pale plinth (`c7dcd0`/`9babb2`/`7f708a`) with a glowing `8fd3ff` groove, moss clumps and tufts, `374e4a` cast shadow; **monolith** = dark slate obelisk (`7f708a`/`625565`/`3e3546`), bevelled top, glowing seam and round core (`ffffff`/`8fd3ff`/`4d9be6`); **ring** = pale stone ring on a neck with a floating orb and motes. Fits the GDD: shrines are the world's mind keeping Awa alive, so they look too clean and too advanced for the countryside. The glow moves from white-green to pale cyan `8fd3ff`, which reads as technology. Godot 2D light adds the real glow in Phase 1. Stamps via `tools/make_stamp.py`. Raw and remapped PixelLab variants kept in `art/raw/shrine/` and `art/final/shrine/` as a record. Review: `art/mocks/review_v2_shrine_6x.png` (monolith bottom left, ring top right), drained `review_v2_shrine_drain_6x.png` |
...
| `ts_ruin16` | Crumbled ruin-wall kit, 16-bit | 16px tiles | — (0 generations) | 3 per type | kit + rubble | 0 | approved | **Approved by Tom, 2026-09-25.** Built from the cottage kit's north-star stone by `tools/kits/make_ruin16.py` → 84 tiles in `art/final/tiles/ruin16/`, index `tools/kits/ruin16.json`. Column types `full high mid low stub end_l end_r`, variants a/b/c, each 1×2 tiles, plus `_shadow` versions for a run's last column. Broken tops: seeded random walk in stone-sized 3–6px steps with drops and missing chunks, `2e222f` outline, pale `ab947a`/`966c6c` cap, 4–7 moss clumps and 1–2 ivy strands; base keeps the stone's grass tufts. **Weathered lower courses** (Tom asked for them): below a boundary that wanders around row 20 per column, stone steps one darker (`ab947a`→`966c6c`, `966c6c`→`625565`) all the way to the base, randomly dithered just above, with damp `374e4a` moss at the foot (first try left a pale plinth at the base and a regular checker line; both fixed). **Rubble stamps** `rubble_a/b/c` (1×1) and `rubble_pile` (2×1): fallen stones with a pale top, stone body, darker base, outline, a shadow down-right and the odd moss dab (`art/final/obj_rubble16_*.png` → `tools/make_stamp.py`). Iterations: linear steps read as a staircase; 1–2px steps read as spikes; a full cottage shadow column read as a slab, now a slim shadow down the last column's right edge. East–west runs only (Art Bible §5.3). Paint: `tools/paint_tiles.py layout ruin COL ROW TYPE...`. Review: `art/mocks/review_v2_ruins_6x.png` |
| `enm_meadow01` | Meadow creature ("mossling") | 24×24 frame, body 17×19 | pixflux ddbce19b, e73bfcfd, 12fd2ced, 37958fec | 4 generated + 2 hand-drawn eye styles | **pupil** | 4 | approved | **Approved by Tom, 2026-09-25 (pupil eye).** Final files `art/final/enm_meadow01_{s,e,n,w}.png`. Sets the enemy signature. PixelLab round (32×32, `no_background`, north star as `color_image_url`, seeds 21–24): 2× "tiny round moss-ball meadow creature … a single large round white eye glowing softly", 2× "tiny flower-bud meadow creature … single large round white eye"; muddy (yellow `d5e04b` blotches, messy eye, bud eye too small), so not used. **Drawn in code** by `tools/kits/make_creature16.py`: a round olive moss ball (`676633` body, `374e4a` shade lower right, so it stands out from the `547e64` grass), a two-leaf sprout, one pale `fdcbb0` petal, stubby feet, soft shadow, and **one large white eye** (`ffffff`, `c7dcd0` rim) as the family signature. Two eye styles: **pupil** (2×2 dark pupil: cute, alive) and **blank** (no pupil: faintly strange). 4 directions (the north view has no eye; west = east flipped). Files `art/final/enm/enm_meadow01_{pupil,blank}_{s,e,n,w}.png`; raw PixelLab variants kept in `art/raw/enm/`. Review: `art/mocks/review_v2_creature_6x.png` (pupil left, blank right), drained `review_v2_creature_drain_6x.png` (the eye stays the brightest thing) |
| `mock_village_full` | Full village mock (Phase 0 exit mock) | 320×180 | — (0 generations) | 1 + drained | — | 0 | review | Built only from approved assets by `tools/mock_layouts/build_v2_village_full.sh` (terrain map `map_v2_village_full.txt`), and matched against the north star (`art/mocks/review_north_star_r2_mix_6x.png`): a row of oaks cut off by the top edge behind the roofs, two 16px-kit cottages with 3-row roofs and chimneys, 1-tile lanes (vertical lane plus a village lane, a grass strip along the wall bases), `shrine_monolith` in a fenced nook between the cottages, a grass band with a standing stone, bushes and flowers above the river ledge, Awa on the lane, the miller outside his door and two mosslings. Iterations: first pass was a corner forest clump and shallow roofs (too empty); a 2-tile lane pass was too wide (the north star lanes are ~1 tile). Drained version excludes Awa (`nodrain`); the creatures' eyes and the shrine glow still read. Known gaps against the north star: no gable-end cottage piece in the 16px kit yet (the north star's left cottage), and the lane can't cross the river ledge yet. Files `art/mocks/v2_village_full_6x.png`, `v2_village_full_drain_6x.png` |
